...
const ReEntryChip: React.FC<IReEntryChipProps> = ({ onClickFn, onDeleteFn, labelText }) => {
  ...
  ReEnterSeedStep: React.FC<IStepProps> = ({ stepForwardFn }) => {
    const { accountSeed } = useAccount(),
      [reEntryText, setReEntryText] = useState<Array<string>>(),
      [isReEnteredCorrectly, setIsReEnteredCorrectly] = useState(false),
      [wordList, setWordList] = useState<Array<string>>(),
      isSmallBrowser = useBreakpoint("<", "sm"),
      [reEntryCounter, setReEntryCounter] = useState<number | undefined>(0),
      /*
       * DEV USE
      ...
      reEntryDeleteFn = useCallback((label: string) => {
        let didChange = false;
        setReEntryText((prev) => {
      ...
      reEntryChipClickFn = useCallback((label: string) => {
        let didChange = false;
        setReEntryText((prev) => {
          let newArray: Array<string> = [];
          if (prev === undefined) {
       ...
       */
      WordChips: Array<ReactElement> | React.ReactFragment = useMemo(() => {
    ...
    return (
      <>
        ...
        <StyledGridContainer
          sx={{ width: isSmallBrowser === true ? "100%" : "80%" }}
          spacing={0}
          container
  ...
  StyledGridContainer = styled(Grid)(({ theme }) => ({
